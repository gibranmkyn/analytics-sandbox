...
#dbExecute(conn, 'ALTER TABLE master_index RENAME stock_price_change TO return_adjusted_price')
#dbExecute(conn, 'ALTER TABLE master_index RENAME stock_price_ratio TO price_adjusted_ratio')
```
...
save(sentiment_data, file = 'temp/sentiment_data.rda')
...
select(-c(accession_number, form_type, cik, return_adjusted_price))
  
year_filed <- model_data$year_filed
model_data <- model_data %>% select(-year_filed) # year_filed is removed so it is not considered by the model

model <- lm(price_adjusted_ratio ~., data = model_data, na.action=na.exclude)
  
model_data$predicted_ratio <- stats::predict(model, newdata = model_data %>% select(-c(price_adjusted_ratio)))
model_data$year_filed <- year_filed # add back year_filed as to help grouping
...
ggplot(model_df, aes(x = reorder(rowname, absolute_t_value), y = absolute_t_value, fill=significance)) + geom_bar(stat = "identity") +
  labs(title ='Model Descriptives: Top 10 features', subtitle = 'groupings on feature type',y = 'Absolute t-value', x = 'Features', caption = paste0('R-squared = ', as.character(round(summary(model)$r.squared,3)) )) +
  ylim(0,5) +  coord_flip() +
...
ggplot(company_agg, aes(x = year_filed, y = price_adjusted_ratio, color = gics_sector)) + geom_line(aes(group = company_name)) + coord_cartesian(ylim=c(-50,50)) + facet_wrap(~group, ncol = 4, scales = "free") + labs(title ='Model Performance: Actual vs Model', subtitle = 'groupings on company Level',y = '% Change in Stock Price', x = 'Year Filed', caption = paste0('R-squared = ', company_agg_rsquared))
