...
the_index <- dbGetQuery(conn, 'SELECT accession_number FROM master_index WHERE form_type = "10-K" ORDER BY date_filed LIMIT 10')
...
doc <- read_html('Edgar filings_HTML view/Form 10-Q/4962/4962_10-Q_2013-07-29_0001193125-13-307673.html') %>%
  html_text() %>%
  tolower() %>%
  removePunctuation() %>%
  removeNumbers %>%
...
doc <- regmatches(doc,gregexpr("(?<=basis of presentation).*",doc,perl=TRUE))[[1]] # regex to slice document from chapter "notes to consolidated financial statements"
doc <- regmatches(doc,gregexpr(".*(?<=item exhibits)",doc,perl=TRUE))[[1]][1] # regex to slice document until chapter "exhibits"
